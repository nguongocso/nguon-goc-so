...
// Ánh xạ kết quả chỉ tiêu sang nhãn + màu sắc
const CRITERION_RESULT_MAP: Record<
    string,
    { label: string; className: string }
> = {
    PASS: {
...
const getCriterionResultBadge = (
    result: InspectionRequestDetailCriterion["result"]
) => {
    if (!result) {
        return (
            <Badge
                variant="outline"
                className={`${CRITERION_RESULT_MAP.PENDING.className} border px-2 py-0.5 text-xs font-medium`}
            >
                {CRITERION_RESULT_MAP.PENDING.label}
            </Badge>
        );
    }
    const key = result.passed === true ? "PASS" : "FAIL";
    const config = CRITERION_RESULT_MAP[key];
    return (
        <Badge
            variant="outline"
            className={`${config.className} border px-2 py-0.5 text-xs font-medium`}
        >
            {config.label}
        </Badge>
    );
...
export const InspectionRequestHistoryModal = ({
    ...
    const totalPassed = items.filter((r) => r.status === "PASSED").length;
    ...
    return createPortal(
        ...
        >
            ...
            >
                {/* ── Header ─────────────────────────────────────────────────── */}
                <div
                    className="flex flex-wrap items-center justify-between gap-3 border-b border-emerald-100 bg-gradient-to-r from-emerald-50 via-white to-white px-5 py-4 sm:px-6">
                    ...
                    <div className="flex items-center gap-3">
                        {/* Tóm tắt nhanh */}
                        <div
                            className="hidden items-center gap-2 rounded-lg border border-emerald-100 bg-white px-4 py-2 text-sm md:flex">
                ...
                <div
                    className="flex flex-col gap-3 border-b border-gray-100 bg-muted/30 px-4 py-3 sm:flex-row sm:items-center sm:justify-between sm:px-6">
                ...
                {/* ── Thống kê nhanh lịch sử kiểm nghiệm ──────────────────── */}
                {items.length > 0 && (
                    <div
                        className="flex flex-wrap items-center gap-3 border-b border-gray-100 bg-slate-50/60 px-4 py-2.5 sm:px-6">
            <span className="text-xs font-semibold text-slate-600">
              ...
              <span className="font-bold">{totalPassed}</span>
              đạt
            </span>
                        <span
              ...
              không đạt
                        ...
                        <span
                            className="inline-flex items-center gap-1 rounded-full border border-yellow-200 bg-yellow-50 px-2.5 py-0.5 text-xs font-medium text-yellow-700">
              <span className="font-bold">{totalPending}</span>
              chờ kết quả
            </span>
                        <span className="ml-auto text-xs text-muted-foreground">
              Tổng cộng {items.length} yêu cầu kiểm nghiệm
            </span>
                    </div>
                )}
                ...
                <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-4 py-4 sm:px-6 sm:py-5">
                            ...
                            <TableBody>
                                {loading ? (
                                    ...
                                    filteredItems.map((request, index) => (
                                        <Fragment key={request.testRequestId}>
                                            <TableRow
                            ...
                            {" "}
                                                ...
                                                <TableCell className="text-center">
                                                    <Button
                                                        ...
                                                        onClick={() => toggleExpand(request.testRequestId)}
                                                        title="Xem kết quả từng chỉ tiêu"
                                                    >
                                                        <ChevronDown
                                                            className={`h-4 w-4 transition-transform ${
                                                                expandedId === request.testRequestId
                                                                    ? "rotate-180"
                                                                    : ""
                                                            }`}
                                                        />
                                            ...
                                            {expandedId === request.testRequestId && (
                                                <TableRow className="bg-slate-50/50">
                                                    <TableCell colSpan={canInspect ? 7 : 6} className="px-4 py-3">
                                                        {detailLoading === request.testRequestId ? (
                                                            <div
                                                                className="flex items-center gap-2 text-xs text-muted-foreground">
                                                                <RefreshCw className="h-3 w-3 animate-spin"/>
                                                                Đang tải kết quả chi tiết…
                                                            </div>
                                                        ) : detailCache[request.testRequestId] ? (
                                ...
                                <span className="font-semibold text-slate-700">
                                  ...
                                  {detailCache[request.testRequestId].passedCriteria}/
                                                                        {detailCache[request.testRequestId].totalCriteria}{" "}
                                                                        đạt
                                </span>
                                                                </div>
                                                                <div
                                                                    className="grid grid-cols-1 gap-1.5 sm:grid-cols-2 lg:grid-cols-3">
                                                                    {detailCache[request.testRequestId].criteria.map(
                                                                        (c) => (
                                                                            <div
                                                                                key={c.criterionId}
                                                                                ...
                                                                                <div className="min-w-0">
                                                                                    ...
                                                                                    {c.result?.resultDate && (
                                                                                        <div
                                                                                            ...
                                                                                            {formatDateOnly(c.result.resultDate)}
                                                                                        </div>
                                                                                    )}
                                                                                </div>
                                                                                {getCriterionResultBadge(c.result)}
                                                                            </div>
                                                                        )
                                                                    )}
                                                                </div>
                    ...
                    <div
                        className="flex flex-wrap items-center justify-between gap-3 pb-1 text-xs text-muted-foreground sm:text-sm">
                        <div>
