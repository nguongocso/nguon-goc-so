...
/**
 * Form tạo mới thành viên trực thuộc tổ chức.
 */
export function CreateOrganizationMemberForm({
  onSubmit,
  loading = false,
  organizationType,
}: CreateOrganizationMemberFormProps) {
  ...
  useEffect(() => {
    ...
    const loadRoles = async () => {
      ...
      setRoleOptions(
        roles.filter((role) =>
          COOPERATIVE_ROLE_CODES.includes(role.code as (typeof COOPERATIVE_ROLE_CODES)[number]),
        ),
      );
    };
