...
import { addExistingUser, getAvailableUsers } from '@/api/organizationApi';
...
/**
 * Dialog tìm kiếm và thêm tài khoản người dùng đã tồn tại vào tổ chức.
 */
export function AddExistingUserDialog({
  ...
  const handleAddUser = async (userId: string) => {
    try {
      ...
      toast.error(axiosError.response?.data?.message || 'Thêm user thất bại');
    } finally {
      setSubmitting(null);
    }
  };
                    ...
                    <TableRow key={user.userId}>
                      ...
                      <TableCell>
                        {availableRoles.length > 0 ? (
                          <Select
                            value={
                              selectedRole[user.userId] ? String(selectedRole[user.userId]) : ''
                            }
                            onValueChange={(val) => {
                      ...
                      <TableCell>
                        <Button
                          size="sm"
                          onClick={() => void handleAddUser(user.userId)}
