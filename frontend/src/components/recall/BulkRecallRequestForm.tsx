...
// Các lựa chọn số dòng mỗi trang của bảng phạm vi thu hồi
...
export const BulkRecallRequestForm: React.FC<BulkRecallRequestFormProps> = ({
  ...
  const statusDisplay = (status: string) =>
    LOT_STATUS_MAP[status] || { label: status, tone: 'neutral' as const };
                ...
                {pagedItems.length === 0 ? (
                  ...
                  pagedItems.map((item, index) => {
                    ...
                    return (
                      <tr
                        key={item.shipmentId}
                        className={item.isRecalled || item.isSplitParent ? 'bg-red-50/50' : ''}
                      >
                        ...
                        <td className="px-3 py-2 text-center text-slate-500">{serialNumber}</td>
                        <td className="px-3 py-2 font-medium text-slate-900">{item.shipmentName}</td>
                        <td className="px-3 py-2">
                          <span
                        ...
                        <td className="px-3 py-2 text-slate-600">{item.organizationName || '—'}</td>
                        <td className="px-3 py-2">
                          {item.isRecalled || item.isSplitParent ? (
                            <span className="text-sm text-slate-500">{item.exclusionReason}</span>
                          ) : !item.included ? (
                            <Input
                              placeholder="Nhập lý do loại..."
                              value={item.exclusionReason}
                              onChange={(e) =>
                                updateExclusionReason(item.shipmentId, e.target.value)
                              }
                              className="text-xs"
                              disabled={creating}
                            />
