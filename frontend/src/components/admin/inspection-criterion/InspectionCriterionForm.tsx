...
const formSchema = z.object({
  ...
  standardId: z.string().min(1, "Vui lòng chọn tiêu chuẩn tham chiếu."),
});
...
export const InspectionCriterionForm = ({
  ...
  // Load danh mục tiêu chuẩn chất lượng mỗi lần mở dialog
  // (không gọi API khi dialog đóng để tránh duplicate call)
  useEffect(() => {
    if (!open) return;
    let cancelled = false;
  ...
  // Reset form mỗi lần mở dialog để tránh giữ giá trị/lỗi cũ từ lần mở trước
  useEffect(() => {
    if (!open) return;
    if (criterion) {
      reset({
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          ...
          <div className="flex flex-col gap-1.5">
            <Label htmlFor="standardId" className="text-sm font-medium">
              Tiêu chuẩn tham chiếu <span className="text-red-500">*</span>
            </Label>
            <Controller
              name="standardId"
              control={control}
            ...
            {standardsError && (
              <p className="text-sm text-red-500">
                ...
                <button
                  type="button"
                  onClick={() =>
                    setStandardsRetryToken((token) => token + 1)
                  }
            ...
            <p className="text-xs text-muted-foreground">
              Tên chỉ tiêu không được trùng trong cùng một tiêu chuẩn tham chiếu.
            </p>
          </div>
