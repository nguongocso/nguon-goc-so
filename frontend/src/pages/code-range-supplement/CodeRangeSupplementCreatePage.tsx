...
export const CodeRangeSupplementCreatePage = () => {
    ...
    <div className="space-y-6">
        ...
        <div className="rounded-xl border border-slate-200 bg-slate-50/70 p-4 text-sm space-y-2">
          <div className="flex items-center justify-between">
            ...
            {quotaLoading ? (
              ...
              <span className="font-semibold text-amber-600">Chưa được cấp dải mã</span>
            ) : (
        ...
        <div className="space-y-1.5">
          ...
          {eventsLoading ? (
            ...
            <div className="space-y-2 rounded-lg border p-3">
              {events.map((event) => (
                <label key={event.id} className="flex cursor-pointer items-start gap-2 text-sm">
                  <input
                    type="checkbox"
                    className="mt-1"
                  ...
                  <span>
                    ...
                    <span className="text-muted-foreground">
                      {' '}
                      — {new Date(event.recordedAt).toLocaleString('vi-VN')} — {event.recordedByName}
                    </span>
      ...
      <Card className="rounded-xl border-slate-200 bg-white shadow-sm">
        <CardContent className="space-y-4 pt-6">
          <h2 className="text-base font-semibold text-slate-900">Yêu cầu của tổ chức</h2>
          <DataTableShell
            colSpan={5}
            header={
              <>
            ...
            body={myRequests.map((item, index) => (
              <TableRow key={item.id} className="hover:bg-muted/40 transition-colors">
                ...
                <TableCell className="font-medium">{item.requestedQuantity.toLocaleString()}</TableCell>
                <TableCell>
                ...
                <TableCell>
                  <StatusBadge label={MY_STATUS_LABEL[item.status]} tone={MY_STATUS_TONE[item.status]} />
                </TableCell>
              </TableRow>
            ))}
            loading={myLoading}
