...
import { SentHandoverListPage } from '../SentHandoverListPage';
...
const mockHandovers: HandoverSummary[] = [
  ...
  },
  {
    ...
    fromOrganizationName: 'HTX Nông Nghiệp Ba Mọi',
    toOrganizationName: 'Hệ Thống Siêu Thị Co.opmart',
    quantity: 1200,
    unit: 'kg',
    status: 'ACCEPTED',
...
function DetailStub() {
  ...
  return <div>Chi tiết phiếu đã gửi: {id}</div>;
}

function renderPage() {
  return render(
    <MemoryRouter initialEntries={['/handover/sent']}>
      <Routes>
        <Route path="/handover/sent" element={<SentHandoverListPage />} />
...
describe('SentHandoverListPage - Danh sách phiếu bàn giao đã gửi cho VT-02', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it('hiển thị danh sách phiếu bàn giao đã gửi với cột Tổ chức nhận và không để lộ UUID', async () => {
    ...
    // Tiêu đề trang
    expect(await screen.findByText('Phiếu bàn giao đã gửi')).toBeInTheDocument();

    // Header bảng
    expect(screen.getByText('STT')).toBeInTheDocument();
    expect(screen.getByText('Tên lô hàng')).toBeInTheDocument();
    expect(screen.getByText('Tổ chức nhận')).toBeInTheDocument();
    ...
    expect(screen.getByText('Công ty Thực Phẩm Sạch VinEco')).toBeInTheDocument();
    ...
    expect(screen.getByText('Hệ Thống Siêu Thị Co.opmart')).toBeInTheDocument();
  ...
  it('click vào dòng bảng điều hướng tới /handover/:id', async () => {
    ...
    expect(await screen.findByText('Chi tiết phiếu đã gửi: 4ee95ecb-978f-42f7-8b09-cf1a966872d1')).toBeInTheDocument();
  });
});
