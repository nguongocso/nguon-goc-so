...
public interface TraceCodeScanLogRepository extends JpaRepository<TraceCodeScanLog, UUID> {
    ...
    @Query("SELECT " +
            "CONCAT(CAST(YEAR(l.scannedAt) AS string), '-', " +
            "CASE WHEN MONTH(l.scannedAt) < 10 THEN '0' ELSE '' END, CAST(MONTH(l.scannedAt) AS string), '-', " +
            "CASE WHEN DAY(l.scannedAt) < 10 THEN '0' ELSE '' END, CAST(DAY(l.scannedAt) AS string)) AS period, " +
            "COUNT(l.id) AS scan_count " +
            "FROM TraceCodeScanLog l " +
            "JOIN l.traceCode tc JOIN tc.shipment s " +
            "WHERE (:orgId IS NULL OR s.organization.organizationId = :orgId) " +
            "AND (:lotId IS NULL OR s.productionLot.id = :lotId) " +
            "AND (:shipmentId IS NULL OR s.id = :shipmentId) " +
            "AND (CAST(:startDate AS date) IS NULL OR l.scannedAt >= :startDate) " +
            "AND (CAST(:endDate AS date) IS NULL OR l.scannedAt <= :endDate) " +
            "GROUP BY YEAR(l.scannedAt), MONTH(l.scannedAt), DAY(l.scannedAt) " +
            "ORDER BY YEAR(l.scannedAt) ASC, MONTH(l.scannedAt) ASC, DAY(l.scannedAt) ASC")
    ...
    @Query("SELECT " +
            "CONCAT(CAST(YEAR(l.scannedAt) AS string), '-W', " +
            "CASE WHEN WEEK(l.scannedAt) < 10 THEN '0' ELSE '' END, CAST(WEEK(l.scannedAt) AS string)) AS period, " +
            "COUNT(l.id) AS scan_count " +
            "FROM TraceCodeScanLog l " +
            "JOIN l.traceCode tc JOIN tc.shipment s " +
            "WHERE (:orgId IS NULL OR s.organization.organizationId = :orgId) " +
            ...
            "AND (CAST(:startDate AS date) IS NULL OR l.scannedAt >= :startDate) " +
            "AND (CAST(:endDate AS date) IS NULL OR l.scannedAt <= :endDate) " +
            "GROUP BY YEAR(l.scannedAt), WEEK(l.scannedAt) " +
            "ORDER BY YEAR(l.scannedAt) ASC, WEEK(l.scannedAt) ASC")
    ...
    @Query("SELECT " +
            "CONCAT(CAST(YEAR(l.scannedAt) AS string), '-', " +
            "CASE WHEN MONTH(l.scannedAt) < 10 THEN '0' ELSE '' END, CAST(MONTH(l.scannedAt) AS string)) AS period, " +
            "COUNT(l.id) AS scan_count " +
            "FROM TraceCodeScanLog l " +
            "JOIN l.traceCode tc JOIN tc.shipment s " +
            "WHERE (:orgId IS NULL OR s.organization.organizationId = :orgId) " +
            ...
            "AND (CAST(:startDate AS date) IS NULL OR l.scannedAt >= :startDate) " +
            "AND (CAST(:endDate AS date) IS NULL OR l.scannedAt <= :endDate) " +
            "GROUP BY YEAR(l.scannedAt), MONTH(l.scannedAt) " +
            "ORDER BY YEAR(l.scannedAt) ASC, MONTH(l.scannedAt) ASC")
    ...
    @Query("SELECT " +
            "CAST(YEAR(l.scannedAt) AS string) AS period, " +
            "COUNT(l.id) AS scan_count " +
            "FROM TraceCodeScanLog l " +
            "JOIN l.traceCode tc JOIN tc.shipment s " +
            "WHERE (:orgId IS NULL OR s.organization.organizationId = :orgId) " +
            ...
            "AND (CAST(:startDate AS date) IS NULL OR l.scannedAt >= :startDate) " +
            "AND (CAST(:endDate AS date) IS NULL OR l.scannedAt <= :endDate) " +
            "GROUP BY YEAR(l.scannedAt) " +
            "ORDER BY YEAR(l.scannedAt) ASC")
