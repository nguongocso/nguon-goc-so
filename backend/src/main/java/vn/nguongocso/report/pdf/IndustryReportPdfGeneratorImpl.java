...
public class IndustryReportPdfGeneratorImpl implements IndustryReportPdfGenerator {
    ...
    public byte[] generate(IndustryReportResponse report) {
        try (ByteArrayOutputStream outputStream = new ByteArrayOutputStream()) {
            ...
            addGeneralInformation(document, report, titleFont, normalFont);
            document.add(buildProductTable(report, headerFont, normalFont));
            document.close();
    ...
    private void addGeneralInformation(Document document, IndustryReportResponse report, Font titleFont, Font normalFont)
        ...
        document.add(new Paragraph("Địa bàn: " + report.getRegion(), normalFont));
        document.add(new Paragraph(
                "Thời gian: "
        ...
        document.add(new Paragraph("Tổng tổ chức: " + report.getTotalOrganizations(), normalFont));
        document.add(new Paragraph("Tổng lô hàng: " + report.getTotalShipments(), normalFont));
        document.add(new Paragraph(
                "Tổng sản lượng: " + NUMBER_FORMAT.format(report.getTotalQuantity()) + " kg",
                normalFont
        ));
        document.add(new Paragraph(" "));
    }
    ...
    private PdfPTable buildProductTable(IndustryReportResponse report, Font headerFont, Font normalFont)
        ...
        addHeader(table, "Loại nông sản", headerFont);
        addHeader(table, "Số lô hàng", headerFont);
        addHeader(table, "Tổng sản lượng (kg)", headerFont);
        table.setHeaderRows(1);

        List<ProductBreakdownItem> items = report.getProductBreakdown();
        if (items == null || items.isEmpty()) {
            PdfPCell cell = new PdfPCell(new Phrase("Không có dữ liệu", normalFont));
            cell.setColspan(3);
            cell.setPadding(8F);
        ...
        for (ProductBreakdownItem item : items) {
            table.addCell(new Phrase(item.getProductCategoryName(), normalFont));
            table.addCell(new Phrase(String.valueOf(item.getShipmentCount()), normalFont));
            table.addCell(new Phrase(NUMBER_FORMAT.format(item.getTotalQuantity()), normalFont));
        }
