...
/** Service gửi thông báo quy trình yêu cầu thu hồi hàng loạt. */
@Slf4j
@Service
...
public class BulkRecallNotificationService {
    ...
    public void sendBulkRecallWorkflowNotification(CustomUserDetails actor, String action, BulkRecallRequest request,
            UUID requestId) {
        ...
        if (organizationId == null) {
            log.warn("Không thể xác định tổ chức cho userId={}", actor.getUserId());
            return;
        }

        List<User> recipients = resolveEligibleManagers(organizationId, actor.getUserId());

        if (recipients.isEmpty()) {
            log.warn("Không có quản lý phù hợp để nhận thông báo trong organizationId={}", organizationId);
            return;
        }

        String title = buildNotificationTitle(action);
        String content = buildNotificationContent(actor.getFullName(), action, request);
    ...
    public void sendBulkRecallNotificationToPurchasingBusiness(BulkRecallRequest request) {

        if (request == null) {
            log.warn("Không thể gửi thông báo đến doanh nghiệp thu mua: request null");
            return;
        }
    ...
    private List<User> resolveEligibleManagers(UUID organizationId, UUID actorId) {
    ...
    private String buildNotificationTitle(String action) {
        ...
        return switch (action) {
            case "CREATE" -> "Yêu cầu thu hồi hàng loạt mới";
            case "APPROVE" -> "Yêu cầu thu hồi hàng loạt được phê duyệt";
            case "REJECT" -> "Yêu cầu thu hồi hàng loạt bị từ chối";
            default -> "Cập nhật yêu cầu thu hồi hàng loạt";
        };
    }

    /** Xây dựng nội dung thông báo theo hành động. */
    private String buildNotificationContent(String actorFullName, String action, BulkRecallRequest request) {
        ...
        return switch (action) {
            ...
            case "REJECT" -> {
                ...
                if (request.getRejectionReason() != null
                    ...
                    yield baseContent + " Lý do: " + request.getRejectionReason();
                }
