...
/**
 * Repository cho mốc canh tác hợp nhất (NCL-09-CN-011).
 */
public interface CultivationMilestoneRepository
          extends JpaRepository<CultivationMilestone, Long> {
     /**
      * Kiểm tra trùng tên trong cùng (loại sản phẩm, tiêu chuẩn).
      * NULL biểu thị áp dụng toàn bộ loại sản phẩm hoặc mọi tiêu chuẩn.
      */
     @Query("""
               SELECT COUNT(m) > 0 FROM CultivationMilestone m
     ...
     /**
      * Tìm mốc canh tác theo bộ lọc và phân trang.
      */
     @Query("""
               SELECT m FROM CultivationMilestone m
     ...
     /**
      * Lấy toàn bộ danh sách mốc canh tác kèm chi tiết loại sản phẩm và tiêu chuẩn.
      */
     @Query("""
               SELECT m FROM CultivationMilestone m
               LEFT JOIN FETCH m.productCategory pc
     ...
     /**
      * Lấy danh sách mốc canh tác bắt buộc để kiểm tra khi đóng gói lô sản xuất.
      */
     @Query("""
               SELECT m FROM CultivationMilestone m
