...
public class InspectionCriterionCatalogServiceImpl implements InspectionCriterionCatalogService {
    ...
    private static final String MSG_DUPLICATE_CRITERION = "Chỉ tiêu với tên và tiêu chuẩn tham chiếu này đã tồn tại.";
    private static final String MSG_REFERENCED_CRITERION = "Chỉ tiêu đang được yêu cầu kiểm nghiệm tham chiếu, không thể xóa.";
    private static final String MSG_THRESHOLD_INVALID = "Ngưỡng tối đa phải lớn hơn 0.";
    private static final String MSG_INVALID_STATUS = "Trạng thái không hợp lệ. Giá trị hợp lệ: ACTIVE, INACTIVE.";
    ...
    /**
     * Tạo mới chỉ tiêu kiểm nghiệm vào danh mục dùng chung.
     */
    @Override
    public InspectionCriterionCatalogResponse createCriterion(
        ...
        String referenceStandard = request.getReferenceStandard() != null
                ? request.getReferenceStandard().trim()
                : null;
        ...
        InspectionCriterionCatalog entity = InspectionCriterionCatalog.builder()
                .name(name)
                .nameEn(request.getNameEn() != null && !request.getNameEn().trim().isBlank()
    ...
    public InspectionCriterionCatalogResponse updateCriterion(
        ...
        entity.setName(name);
        entity.setNameEn(
                request.getNameEn() != null && !request.getNameEn().trim().isBlank() ? request.getNameEn().trim()
                        : null);
        entity.setUnit(unit);
    ...
    /**
     * Xóa chỉ tiêu kiểm nghiệm khỏi danh mục dùng chung.
     */
    @Override
    ...
    /**
     * Kiểm tra ngưỡng tối đa của chỉ tiêu kiểm nghiệm.
     */
