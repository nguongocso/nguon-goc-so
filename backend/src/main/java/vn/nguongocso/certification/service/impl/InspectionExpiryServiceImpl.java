...
public class InspectionExpiryServiceImpl implements InspectionExpiryService {
    ...
    public boolean checkAndAlertLotExpiry(ProductionLot lot, LocalDate today) {
        ...
        InspectionValidityStatus status = validity.getStatus();
        if (status == InspectionValidityStatus.VALID) {
            autoResolveAllPendingInspectionAlerts(lot.getId());
            return false;
        }
    ...
    private void autoResolveAllPendingInspectionAlerts(UUID lotId) {
        ...
        autoResolveExpiringAlert(lotId);
        List<Alert> pendingExpiredAlerts = alertRepository.findByRelatedEntityIdAndTypeAndStatus(
                lotId,
                AlertType.INSPECTION_EXPIRED,
                AlertStatus.PENDING);

        for (Alert alert : pendingExpiredAlerts) {
            alert.setStatus(AlertStatus.RESOLVED);
            alert.setResolvedAt(LocalDateTime.now());
            alertRepository.save(alert);
            log.info("⚙️ Tự động RESOLVED cảnh báo hết hiệu lực của lô ID: {}", lotId);
        }
    }
