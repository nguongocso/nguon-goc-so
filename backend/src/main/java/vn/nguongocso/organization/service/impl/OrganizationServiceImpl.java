...
public class OrganizationServiceImpl implements OrganizationService {
    ...
    public OrganizationResponse createOrganization(CreateOrganizationRequest request) {
        ...
        log.info("Tạo organization thành công. organizationId={}, manager={}",
                organization.getOrganizationId(), manager.getUserName());
    ...
    public List<OrganizationResponse> getAllOrganizations() {
        log.info("Bắt đầu lấy danh sách organization");
     ...
     * Chỉ trả các tổ chức ACTIVE thuộc Doanh nghiệp thu mua (VT-04, loại ENTERPRISE)
    ...
    public OrganizationProfileResponse updateCurrentOrganization(OrganizationUpdateRequest request) {
        ...
        log.info("Cập nhật hồ sơ tổ chức thành công: orgId={}", organizationId);
        return toProfileResponse(organization);
    }
    ...
    @Override
    @Transactional(readOnly = true)
    public OrganizationDetailResponse getOrganizationDetail(UUID organizationId) {
        Organization organization = organizationRepository.findById(organizationId)
                .orElseThrow(() -> new BusinessException("Tổ chức không tồn tại"));
    ...
    @Override
    @Transactional
    public CreateOrganizationMemberResponse addMember(UUID organizationId, AddMemberRequest request) {
    ...
    /**
     * Gán vai trò mới cho thành viên trong tổ chức hiện tại.
     *
     * @param request thông tin gán vai trò
     * @return thông tin thành viên sau khi gán
     */
    @Override
    @Transactional
        ...
        return availableUsers.stream()
                .map(user -> {
                    ...
                    return AvailableUserResponse.builder()
                            ...
                            .email(user.getEmail())
                            .phone(user.getPhone())
                            .currentRoleCode(orgUser != null ? orgUser.getRole().getCode() : null)
                            .currentRoleName(orgUser != null ? orgUser.getRole().getName() : null)
                            .build();
                })
                .collect(Collectors.toList());
    }
    ...
    @Override
    @Transactional
    public OrganizationUserResponse addExistingUserToOrganization(
            UUID organizationId,
            UUID userId,
            Integer roleId) {
        ...
        if (roleId != null) {
            ...
            OrganizationUser currentOrgUser = organizationUserRepository
                    .findFirstByUserAndOrganization_Type(user, org.getType())
                    .orElseThrow(() -> new BusinessException("Không tìm thấy role hiện tại của user"));
    ...
    private void assignManagerRole(Organization organization, User manager) {
        ...
        log.debug("Gán role {} cho user {} trong organization {}",
                role.getCode(), manager.getUserName(), organization.getCode());
    ...
    private OrganizationProfileResponse toProfileResponse(Organization organization) {
    ...
    private OrganizationUserResponse toOrganizationUserResponse(OrganizationUser organizationUser) {
        User user = organizationUser.getUser();
        Role role = organizationUser.getRole();

        return OrganizationUserResponse.builder()
                .id(organizationUser.getId())
                .organizationId(organizationUser.getOrganization().getOrganizationId())
                .userId(user.getUserId())
                .username(user.getUserName())
                .fullName(user.getFullName())
                .email(user.getEmail())
