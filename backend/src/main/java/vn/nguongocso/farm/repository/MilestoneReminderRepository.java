...
public interface MilestoneReminderRepository extends JpaRepository<MilestoneReminder, UUID> {
    /** Kiểm tra nhắc việc của mốc và lô đã tạo trong ngày hay chưa. */
    boolean existsByProductionLot_IdAndMilestone_IdAndReminderDate(
            UUID lotId,
            Long milestoneId,
            LocalDate reminderDate);

    /** Kiểm tra nhắc việc của mốc, lô và người nhận đã tạo trong ngày hay chưa. */
    ...
    /** Tìm nhắc việc mở cho người dùng hoặc các lô thuộc tổ chức. */
    ...
    /** Tìm các nhắc việc của người dùng theo trạng thái theo số ngày quá hạn. */
    List<MilestoneReminder> findByUser_UserIdAndStatusOrderByOverdueDaysDesc(
            UUID userId,
            MilestoneReminderStatus status);

    /** Tìm danh sách nhắc việc của tổ chức theo trạng thái theo số ngày quá hạn. */
