    ...
    public UUID lockAccount(
        ...
        if (activeLock != null) {
            ...
            if (!activeLock.isPermanent() && activeLock.getLockUntil() != null) {
                ...
                log.info("Cập nhật bản ghi khóa cũ từ thời hạn hết hạn thành UNLOCKED. accountId={}, elapsedSeconds={}",
                        accountId, elapsedSeconds);
            } else if (activeLock.isPermanent() || activeLock.getLockUntil() == null) {
                if (activeLock.isPermanent()) {
                    throw new BusinessException(
                            "Tài khoản đang bị khóa vĩnh viễn, cần mở khóa thủ công mới được truy cập lại");
                }
                ...
                activeLock.setUnlockedAt(now);
                accountLockRepository.save(activeLock);
                log.info("Cập nhật bản ghi khóa cũ từ timeout thành UNLOCKED. accountId={}, elapsedSeconds={}",
                        accountId, elapsedSeconds);
            }
        }
