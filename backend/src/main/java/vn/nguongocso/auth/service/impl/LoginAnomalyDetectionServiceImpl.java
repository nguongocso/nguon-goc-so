...
public class LoginAnomalyDetectionServiceImpl implements LoginAnomalyDetectionService {
     ...
     */
    @Override
    public void recordLoginAttempt(User user, String usernameInput, boolean isSuccess, String ipAddress,
            String countryCode) {
        try {
            if (user == null) {
                log.warn("Bỏ qua ghi nhận lần đăng nhập do user null. usernameInput={}, ipAddress={}", usernameInput,
                        ipAddress);
                return;
            }
    ...
    /**
     * Kích hoạt trường hợp đáng ngờ nếu vượt ngưỡng.
     */
