...
public class LoginMonitoringServiceImpl implements LoginMonitoringService {
    ...
    private static final String VT_01 = "VT-01";
    private static final String VT_02 = "VT-02";
    ...
    public PageResponse<LoginHistoryResponse> getLoginHistory(
            UUID userId, String result, UUID organizationId, String startDate, String endDate, Pageable pageable) {
        ...
        List<LoginHistoryResponse> items = attemptsPage.getContent().stream()
                ...
                .filter(attempt -> startDate == null
                        || !attempt.getCreatedAt().toLocalDate().isBefore(LocalDate.parse(startDate)))
                .filter(attempt -> endDate == null
                        || !attempt.getCreatedAt().toLocalDate().isAfter(LocalDate.parse(endDate)))
                .map(attempt -> {
    ...
    public PageResponse<LoginAnomalyResponse> getLoginAnomalies(
        ...
        List<LoginAnomalyResponse> filteredItems = anomaliesPage.getContent().stream()
                ...
                .filter(anomaly -> normalizedUsername == null || normalizedUsername.isBlank()
                        || anomaly.getUser() != null && (anomaly.getUser().getUserName() != null
                                && anomaly.getUser().getUserName().toLowerCase()
                                        .contains(normalizedUsername.toLowerCase())
                                || anomaly.getUser().getFullName() != null && anomaly.getUser().getFullName()
                                        .toLowerCase().contains(normalizedUsername.toLowerCase())))
                .map(anomaly -> {
                    ...
                    return LoginAnomalyResponse.builder()
                            ...
                            .roleCode(roleCode)
                            .organizationId(
                                    anomaly.getOrganization() != null ? anomaly.getOrganization().getOrganizationId()
                                            : null)
                            .organizationName(
                                    anomaly.getOrganization() != null ? anomaly.getOrganization().getName() : null)
    ...
    /**
     * Lấy danh sách tài khoản nghi vấn.
     */
    @Override
    public PageResponse<SuspiciousCaseResponse> getSuspiciousCases(
            String status, UUID organizationId, String username, Pageable pageable) {
        ...
        List<SuspiciousCaseResponse> filteredItems = casesPage.getContent().stream()
                ...
                .filter(item -> normalizedUsername == null || normalizedUsername.isBlank()
                        || item.getUser() != null && (item.getUser().getUserName() != null
                                && item.getUser().getUserName().toLowerCase().contains(normalizedUsername.toLowerCase())
                                || item.getUser().getFullName() != null && item.getUser().getFullName().toLowerCase()
                                        .contains(normalizedUsername.toLowerCase())))
                .map(item -> SuspiciousCaseResponse.builder()
                        ...
                        .organizationId(
                                item.getOrganization() != null ? item.getOrganization().getOrganizationId() : null)
    ...
    /**
     * Đánh dấu đã giải quyết bất thường của tài khoản.
     */
    @Override
