...
| **BE Service** | `trace/service/ShipmentHandoverService.java` + `impl/ShipmentHandoverServiceImpl.java` (create/cancel/accept/reject/getById/sent/received/remaining/hasPending/uploadAttachment); `ShipmentServiceImpl.getEligibleShipments()` — liệt kê lô cho màn hình Thu mua **scope theo tổ chức hiện tại**: lô có phiếu bàn giao **đang hoạt động** (`PENDING_CONFIRMATION`/`ACCEPTED`) nhắm tới org ∪ lô có sự kiện `PROCUREMENT`/`WAREHOUSE_RECEIPT` do org ghi (`isCorrection=false`); phiếu REJECTED/EXPIRED/CANCELLED bị loại (lô không còn giao dịch sống); `trace/service/HandoverExpiryService.java` + `impl/HandoverExpiryServiceImpl.java` (lazy-expire khi đọc + cron) |
...
| `schema/V20260908000000` | shipment_handovers |
