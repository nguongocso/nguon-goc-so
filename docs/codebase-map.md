...
| **BE Service** | `trace/service/ShipmentHandoverService.java` + `impl/ShipmentHandoverServiceImpl.java` (create/cancel/accept/reject/getById/sent/received/remaining/hasPending/uploadAttachment); `ShipmentServiceImpl.getEligibleShipments()` — liệt kê lô cho màn hình Thu mua **scope theo tổ chức hiện tại**: lô có **phiếu bàn giao MỚI NHẤT** ở trạng thái hoạt động (`PENDING_CONFIRMATION`/`ACCEPTED`) nhắm tới org ∪ lô có sự kiện `PROCUREMENT`/`WAREHOUSE_RECEIPT` do org ghi (`isCorrection=false`); phiếu mới nhất REJECTED/EXPIRED/CANCELLED → lô bị loại kể cả khi có phiếu ACCEPTED cũ (quan hệ hiện tại đã kết thúc); `trace/service/HandoverExpiryService.java` + `impl/HandoverExpiryServiceImpl.java` (lazy-expire khi đọc + cron) |
