...
|---|---|---|---|---|---|---|
| MTC-01 | Mặc định hiển thị đủ 4 tổ chức | VT-01 `admin`, mở `/reports/organization-usage` (dữ liệu tự tải, không cần nhấn nút) | Quan sát bảng | Có 4 dòng: HTXA, HTXB, HTXC, SYSTEM. Đúng tổng (Summary card "Tổng số tổ chức" = 4; ba nhóm trạng thái cộng lại bằng tổng số). Thứ tự mặc định: Cần liên hệ hỗ trợ → Đang hoạt động → Chưa có dữ liệu. **Phía trên bảng có mô tả kỳ báo cáo hai dòng: "Kỳ hiện tại: từ ... đến ..." và "Kỳ trước: từ ... đến ..." (ngày đã chuẩn hóa từ backend, kỳ trước có cùng độ dài kỳ hiện tại).** | — | Cao |
