...
> 1. Nút **"Yêu cầu cấp bổ sung mã"** trong tab **"Lô hàng & Mã QR"** của trang chi tiết lô sản xuất.
> 2. Cảnh báo hạn mức trên màn hình **Tạo lô hàng** (`/production-lots/{lotId}/shipments/create`):
>    - hạn mức còn < 20% (banner vàng),
>    - hạn mức đã hết (banner vàng "Hạn mức đã hết, vui lòng yêu cầu cấp bổ sung mã truy xuất."),
...
|---|---|---|---|
| TC-01 | Tùy chọn trong tab "Lô hàng & Mã QR" | Đăng nhập `manager` → chi tiết lô sản xuất → tab "Lô hàng & Mã QR" | Header có 2 nút: "Yêu cầu cấp bổ sung mã" + "Tạo lô hàng" |
| TC-02 | Mở dialog, hạn mức bình thường (TC-DATA-1) | Bấm "Yêu cầu cấp bổ sung mã" | Dialog mở tại chỗ; hiện "Còn 99.000 / 100.000 mã"; không cảnh báo vàng |
| TC-03 | Cảnh báo hạn mức thấp / hết | Chạy TC-DATA-2 rồi TC-DATA-3, mở lại dialog và trang Tạo lô hàng | <20%: banner vàng cảnh báo trên dialog + trang tạo lô hàng; =0: trang tạo lô hàng khóa form + banner "Hạn mức đã hết, vui lòng yêu cầu cấp bổ sung mã truy xuất." |
| TC-04 | Gửi yêu cầu hợp lệ | Trong dialog: SL = 500, lý do, chọn ≥1 sự kiện Thu hoạch/Sơ chế → Gửi | Toast thành công; dòng mới trong bảng "Yêu cầu của tổ chức" với trạng thái "Chờ duyệt" |
| TC-05 | Validation | Bỏ trống lý do / SL = 0 / chưa chọn sự kiện → Gửi | Chặn gửi, hiện đúng thông báo lỗi zod (lỗi đầu tiên) |
...
| TC-07 | Lối tắt ở trang Tạo lô hàng | Vào Tạo lô hàng khi hạn mức <20% hoặc =0 | Banner vàng có nút "Yêu cầu cấp bổ sung mã" → mở dialog (không rời trang) |
...
| TC-11 | Quyền | Đăng nhập `nguoighi` (VT-03) → tab "Lô hàng & Mã QR" | Không thấy nút "Yêu cầu cấp bổ sung mã"; POST API trực tiếp → 403 |
...
| TC-13 | Nhập số lượng vượt hạn mức | TC-DATA-1 (còn 99.000), vào Tạo lô hàng, nhập SL = 100.000 | Ngay khi nhập hiện **cảnh báo đỏ** "Hạn mức đã hết / không đủ mã. Chỉ còn 99.000 mã truy xuất…" + nút "Yêu cầu cấp bổ sung mã"; nút "Tạo lô hàng" bị khóa |
| TC-14 | Hết hạn mức mở trang tạo lô hàng | TC-DATA-3, mở Tạo lô hàng | Form khóa, banner vàng "Hạn mức đã hết, vui lòng yêu cầu cấp bổ sung mã truy xuất." + nút mở dialog |
| TC-15 | Giảm số lượng về mức hợp lệ | TC-13 → sửa SL xuống 1.000 | Cảnh báo đỏ tự ẩn, tạo lô hàng bình thường |
| TC-16 | Nút trong cảnh báo mở dialog | TC-13 → bấm "Yêu cầu cấp bổ sung mã" trong cảnh báo đỏ | Dialog mở đúng tại trang, không điều hướng |
